...
function UserManager({users,onUserCreated,currentUserId,tab,onTabChange,onArchive,onRestore,onDelete}) {
  ...
  return (
    <div>
      ...
      <div style={{background:C.card,borderRadius:13,overflow:"hidden",boxShadow:"0 1px 3px rgba(0,0,0,.05)"}}>
        {shown.map((u,i)=>(
          <div key={u.id} style={{padding:"12px 18px",borderBottom:i<shown.length-1?`1px solid ${C.border}`:"none",display:"flex",alignItems:"center",gap:12}}>
...
export default function App() {
  ...
  // Archived engineers drop out of new-booking/reassignment pickers (see
  // JobForm/ReassignModal below) but `engineers` itself stays the full list
  // everywhere else -- job history, the dashboard, and Payments all still
  // need to resolve/show an already-assigned engineer's name even after
  // they've been archived.
  const assignableEngineers = engineers.filter(e=>!e.archivedAt);
  ...
  const doReassign=async (engId)=>{
    const eng=engineers.find(e=>e.id===engId);
    await supabase.from("bookings").update({
      engineer_id: engId||null, status: engId?"Assigned":"Booked", rate: eng?eng.rate:reassign.rate,
    }).eq("id",reassign.id);
    await loadBookings();
            ...
            <div>
              ...
              {engTab==="archived" ? (
                <div style={{background:C.card,borderRadius:13,overflow:"hidden",boxShadow:"0 1px 3px rgba(0,0,0,.05)"}}>
                  ...
                  {shown.map((eng,i)=>(
                    <div key={eng.id} style={{padding:"12px 18px",borderBottom:i<shown.length-1?`1px solid ${C.border}`:"none",display:"flex",alignItems:"center",gap:12}}>
