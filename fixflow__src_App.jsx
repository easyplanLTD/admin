...
function AddEngineerModal({onCreated,onCancel}) {
  ...
  const submit = async () => {
    ...
    setBusy(false);
    if(error||data?.error){ setErr(data?.error||error.message||"Something went wrong creating this engineer."); return; }
    onCreated(data);
  };
...
function UserManager({users,onUserCreated}) {
  ...
  const addUser=async ()=>{
    ...
    setBusy(false);
    if(error||data?.error){ setErr(data?.error||error.message||"Something went wrong creating this user."); return; }
    onUserCreated();
